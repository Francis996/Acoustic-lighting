...
const rows = fs
  ...
  .map((row) => ({
    ...
    branding_review_reason: "Visual review found no logo, brand name, watermark, residue, patch, or structural mismatch in the output.",
...
async function contactSheet(items, filename) {
  ...
  if (!items.length) {
    composites.push({
      input: labelSvg("No manual-review or failed items in this pilot.", width, 120),
      left: 0,
      top: 70
    });
  }
  ...
  for (let i = 0; i < items.length; i += 1) {
    ...
    composites.push({ input: labelSvg(`HD OUTPUT | ${item.branding_review_status} | ${item.processing_status}`), left: cellW, top: top + cellH });
  }
...
await contactSheet(
  rows.filter((row) => row.manual_review_required || row.processing_status.startsWith("failed")),
