...
const rows = fs.readFileSync(manifestPath, "utf8").trim().split("\n").filter(Boolean).map((line) => {
  ...
  const reason = manualReview.get(row.model_id);
  if (reason) {
    return {
      ...row,
...
await contactSheet(rows.filter((row) => row.manual_review_required || row.processing_status.startsWith("failed")), "manual-review-contact-sheet.webp");
